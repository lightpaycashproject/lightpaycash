...
*Setup instructions for a gitian build of LightPayCash using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the LightPayCash
...
- [Building LightPayCash](#building-lightpaycash)
...
Clone the git repositories for lightpaycash and gitian and then checkout the lightpaycash version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/lightpaycash-crypto/lightpaycash.git
cd lightpaycash
...
Follow the instructions in [doc/release-process.md](release-process.md) in the lightpaycash repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building LightPayCash
----------------

To build LightPayCash (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the lightpaycash repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/lightpaycash/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/lightpaycash-crypto/lightpaycash
...
For example:
```bash
URL=https://github.com/crowning-/lightpaycash.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit lightpaycash=${COMMIT} --url lightpaycash=${URL} ../LightPayCash/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit lightpaycash=${COMMIT} --url lightpaycash=${URL} ../LightPayCash/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit lightpaycash=${COMMIT} --url lightpaycash=${URL} ../LightPayCash/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/lightpaycash-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/lightpaycash-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/lightpaycash-build.assert
```
...
[lightpaycash/gitian.sigs](https://github.com/lightpaycash-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
